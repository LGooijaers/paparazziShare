...
/** The old orange filter was commented out an copied in order to adapt it and add a grid.
* In this function the color count is now seperated into 9 cells (3 rows and 3 columns).  
* 
*/


// /**
//  * Filter colors in an YUV422 image for all batches
//  * @param[in] *input The input image to filter
//  * @param[out] *output The filtered output image
//  * @param[in] y_m The Y minimum value
//  * @param[in] y_M The Y maximum value
...
//  * @param[in] v_m The V minimum value
//  * @param[in] v_M The V maximum value
//  * @param[in] *cnt_cells pointer to a 9 cell array that keeps track of the pixel count in each cell
//  * @return void but the count_cells array that already existed is updated inside this function
		 		3 6 9
	shape(cnt_cells) = 	2 5 8
				1 4 7
//  */
...
{
	//reset all pixel counts to zero
  for(int i = 0; i < 9; i++){
    (*(cnt_cells + i)) = 0;
  }
	//copy pointer to image files (still the same)
  uint8_t *source = (uint8_t *)input->buf;
  uint8_t *dest = (uint8_t *)output->buf;
  ...
  output->ts = input->ts;
	...
	// Since the image matrix is rotated 270 degrees it is looped from the left bottom corner to the right top corner by looping from bottom to top sideways
	//The image height is actually its width and vice versa
	//By first looping over x and then y, x and y do represent width and height respectively
	//Since in each loop 2 pixels are checked y values are are always incremented by 2
  for (uint16_t x = 0; x < (output->h); x += 1) {
	...
	//Check in which row the pixel is located by checking the position of the y-value
        if (y< (output->w)/3){
          row = 0;
        }
	...
	//Check in which column the pixel is located by checking the position of the x-value
        if (x< (output->h)/3){
          col = 0;
        }
        else if ((x>=(output->h)/3) && (x < ((output->h)/3) * 2)){
          col = 1;
        }
	...
	//update cnt_cells array by adding 1
        (*(cnt_cells + row + col * 3 ))++;
        // UYVY color the pixel green
        dest[0] = 43;        // U
        dest[1] = source[1];  // Y
        dest[2] = 21;        // V
        dest[3] = source[3];  // Y
      ...
      //If pixel is not in specified value make it grey-scale (same as old function)
      else {
        // UYVY
        char u = source[0] - 127;
        u /= 4;
