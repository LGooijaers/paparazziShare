...
void image_yuv422_colorfilt_cells(struct image_t *input, struct image_t *output, uint8_t y_m, uint8_t y_M, uint8_t u_m,
                                uint8_t u_M, uint8_t v_m, uint8_t v_M, int *rowArray, int *columnArray, int numRows, int numCols, int *cnt_cells)
{
  // uint16_t cnt = 0;
  uint8_t *source = (uint8_t *)input->buf;
  uint8_t *dest = (uint8_t *)output->buf;
  ...
  // Go trough all the pixels
  for(int i = 0; i < (numCols-1); i += 1){            // Note, the range of the forloop is probably not correct, as the loop runs for 1 elemnt too little now
    for(int j = 0; j < (numRows-1); j += 1){
      for (uint16_t y = *rowArray[j]; y < *roawArray[j+1]; y += 1) {
